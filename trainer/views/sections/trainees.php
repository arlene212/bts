...
<section class="tab-content <?php echo $currentTab === 'trainees' ? 'active' : ''; ?>" id="trainees">
  ...
  <div class="search-container"><form method="GET" action="trainer.php#trainees"><input type="hidden" name="trainee_page" value="1"><div class="input-group"><span class="input-group-text"><i class="fas fa-search"></i></span><input type="text" name="trainee_search" class="form-control" placeholder="Search trainees by name, ID, email, or course..." value="<?php echo htmlspecialchars($traineeSearch); ?>"><button type="submit" class="btn btn-primary">Search</button><?php if (!empty($traineeSearch)): ?><a href="trainer.php?trainee_page=1#trainees" class="btn btn-link">Clear</a><?php endif; ?></div></form></div>
  ...
  <?php if ($totalTraineePages > 1): ?><div class="pagination"><?php for ($i = 1; $i <= $totalTraineePages; $i++): ?><a href="?trainee_page=<?php echo $i; ?>&trainee_search=<?php echo urlencode($traineeSearch); ?>#trainees" class="page-link <?php echo $i == $traineePage ? 'active' : ''; ?>"><?php echo $i; ?></a><?php endfor; ?></div><?php endif; ?>
</section>