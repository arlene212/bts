...
<section class="main-content tab-content <?php echo ($currentTab === 'guests' ? 'active' : ''); ?>" id="guests">
  ...
  <div class="table-container">
    <table class="table table-striped table-hover guest-table"><thead><tr><th>User Name</th><th>Email</th><th>Contact Number</th><th>Enrolled Courses</th><th>Date Created</th></tr></thead><tbody>
      <?php if (empty($guests)): ?>
        <tr><td colspan="5" class="no-data">No guests found</td></tr>
      <?php else: ?>
        <?php foreach ($guests as $guest): ?>
          <tr>
            <td><?php echo htmlspecialchars(($guest['last_name'] ?? '') . ', ' . ($guest['first_name'] ?? '') . (!empty($guest['middle_name']) ? ' ' . $guest['middle_name'] . '.' : '')); ?></td>
            ...
            <td><?php echo !empty($guest['date_created']) ? date('Y-m-d', strtotime($guest['date_created'])) : ''; ?></td>
          </tr>
        <?php endforeach; ?>
      <?php endif; ?>
    </tbody></table>
