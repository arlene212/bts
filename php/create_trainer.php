...
try {
    ...
    $dupStmt = $pdo->prepare("SELECT user_id FROM users WHERE first_name = ? AND middle_name = ? AND last_name = ? AND role = 'trainer'");
    $dupStmt->execute([$firstName, $middleName, $lastName]);
    if ($dupStmt->fetch()) {
        http_response_code(409); // Conflict
        echo json_encode(['success' => false, 'message' => 'A trainer with this exact full name already exists.']);
        exit;
    }
